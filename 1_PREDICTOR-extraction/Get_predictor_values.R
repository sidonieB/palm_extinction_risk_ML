...
# by claculating their EOO in two times, one for positive longitudes and one for negative longitudes
# then summing both
###############################################################################
library("ConR")
...
# we will extract the values from low resolution (20km=10min) layers without buffering during the extraction, because the low resultion will already buffer this.
...
# for Pflex, only concerns 78 species in if use min and 5 if use mean (we will use mean in the article)
